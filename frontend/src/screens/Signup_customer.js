...
import { Signup_cust } from '../actions/userActions'
...
const Signup_customer = (props) => {
  ...
  useEffect(() => {
    ...
    if (response && response.status=="OK") {
      ...
      props.history.push('/signin_customer')
    } else if (error) {
  ...
  const onSignup = () => {
    ...
    dispatch(Signup_cust(firstName, lastName, email, password))
    document.location.href = '/signin_customer'
  }
      ...
      <div className="form">
        ...
        <div className="mb-3">
          ...
          <div className="float-end">
            Existing user? <Link to="/Signin_customer">Signin here</Link>
          </div>
        </div>
      </div>
