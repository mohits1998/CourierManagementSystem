...
const MyOrder = () => {
  ...
  useEffect(() => {
    const getMyCourier = async () => {
      const myCouriers = await retrieveMyCouriers();
      if (myCouriers) {
        setMyCouriers(myCouriers);
      }
    };
  ...
  const retrieveMyCouriers = async () => {
    const response = await axios.get(
      "http://localhost:8080/api/user/courier/myCouriers?userId=" + user.id
    );
